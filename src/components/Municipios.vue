<template>
  <div class="wrapper">
      ...
      <div id="container">
         <div class="well well-sm" id="mapa-header"> Mapa {{nom_dashboard}} {{mun[1].charAt(0).toUpperCase() + mun[1].substring(1 , mun[1].length).toLowerCase()}}</div>

         <div id="map"></div>

         <div id="eleccion">

           <div class="well well-sm" style="font-weight:bold; font-size: 19px;"> Datos por {{secciones_mapa[1]}}</div>
          ...
          <div id="grafica">
            <ul class="nav nav-tabs">
              ...
              <li
                ...
                <a data-toggle="tab">{{item }} {{mun[1].charAt(0).toUpperCase() + mun[1].substring(1 , mun[1].length).toLowerCase()}} </a>
              </li>
            </ul>
            <div class="tab-content">
              <div
               ...
               >
                  ...
                  <select name=item :id='item' @change="cargar_datos_comparacion(item, mun_select,item_select_anterior)" v-model="mun_select" class="form-control" style="height:40px; width:100%">
                  ...
                  <div class="dropdown" >
                    <button class="btn btn-secondary dropdown-toggle" type="button" id="dropdownMenuButton" @click="cargar_datos_tabla(item,municipios_s)" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"
...
export default {
    ...
    data() {
      ...
      mounted(){
          ...
          this.cargar_datos_grafica(this.mun);
      ...
      methods: {
             ...
             axios({
                ...
                url: 'http://localhost:3000/api/dashboard/name/TFG'}
                 ...
                 var d = this.patrones[0].replace("XX", year);
                  ...
                  axios({
                     method: 'get',
                     url: 'http://localhost:3000/api/datasets/'+d+'/coordinates/'+this.mun[0]}
                  ).then(response => {
                ...
                for (var key in data){
                      ...
                      if (key == this.nom_dashboard){
                        ...
                        for (var i in data_key){
                          ...
                          axios({
                             method: 'get',
                             url: 'http://localhost:3000/api/datasets/'+patron_year+'/items/'+this.mun[0]}
                           ).then(response => {
                              var data = response.data
                              var suma = 0;
                              var dato = [];
                ...
                var ourCustomLegend = L.Control.extend({
                 options: {
                  ...
                  onAdd: function (map) {
                   ...
                   container.innerHTML +='<div style="background:#D1D1D1"> <span>Datos de las municipios</span>';
         ...
         async cargar_datos_grafica(mun){

              this.municipios_s.push(this.mun);
              ...
              axios({
                ...
                url: 'http://localhost:3000/api/dashboard/name/TFG'}
                 ...
                 this.cargar_datos_series(patrones,mun);
          ...
          async cargar_datos_series(patrones,mun){
              //this.prueba.push(this.chartOp)
              var datos_grafica = [];
              var categorias = [];
              var series= [];
                ...
                for (var chart in this.chartOp){
                    ...
                    if (seccion == nom_chart){
                       ...
                       for(var p in patrones_s){
                          ...
                          urls.push('http://localhost:3000/api/datasets/'+patron_p+'/items/'+mun[0]);
                        ...
                        for (var x in datos_i){
                            ...
                            if (seccion_i.indexOf("c_") !==-1){
                                if (items_comp.indexOf(x) == -1){
                                  ...
                                  chart_op.series.push({name: name_s +" "+  mun[1].charAt(0).toUpperCase() + mun[1].substring(1 , mun[1].length).toLowerCase(), title:x, data:[]})

                                }
                                var chart_series = chart_op.series

                                for (var series in chart_series){
                                  if (chart_series[series].name == name_s+" "+ mun[1].charAt(0).toUpperCase() + mun[1].substring(1 , mun[1].length).toLowerCase()){
         ...
         async cargar_datos_comparacion(item, mun, item_anterior){
              ...
              for (var chart in this.chartOp){
                  ...
                  if (title_chart == item){
                     ...
                     if(this.veces>0){
                         ...
                         if (this.municipios_s.length>1){
                           this.municipios_s.pop();
                            ...
                            for (var i=0; i<results.length; i++){
                                ...
                                for (var d in data){
                                  ...
                                  if (this.items_comp.indexOf(d) == -1){
                                    ...
                                    chart_op.series.push({name: name_s + " "+  mun[1].charAt(0).toUpperCase() + mun[1].substring(1 , mun[1].length).toLowerCase(), title:d, data:[]})
               ...
               this.municipios_s.push(this.mun_select);
               this.mun_select = null;

         },

         async cargar_datos_tabla(item, mun){

            var urls = [];
            var results = [];


            for (var municipios in mun){
                this.prueba.push(prov[municipios]);
                var municipios = mun[municipios]
               ...
               for (var patron in this.patrones_tabla){
                   ...
                   if (seccion.indexOf(item) !==-1){
                       ...
                       for(var p in patrones_s){
                           var patron_p = patrones_s[p];
                           urls.push('http://localhost:3000/api/datasets/'+patron_p+'/data/id/'+municipios[0]);
         ...
         resetear_datos_grafica(item){
              ...
              if (this.seccion_select !==null && this.municipios_s.length>1){
                  for (var chart in this.chartOp){
                      var chart_op = this.chartOp[chart];
                      var title_chart = chart_op.title.text;
                      var nom_chart = chart_op.name;
